...
class VisuraIngestService:
    ...
    def ingest(self, conn, adapter: ItemAdapter, *, locatore_cf: str) -> VisuraIngestResult:
        ...
        if visura_source == "sister" and visura_downloaded:
            ...
            visura_db_id = db_upsert_visura(
                ...
                self.storage.visura_object_name(locatore_cf),
                None,
                fetched_now=False,
            )
...
class ContractSyncService:
    ...
    def sync(self, conn, adapter: ItemAdapter, *, immobile_id: int) -> ContractSyncResult:
...
class AuditStageService:
    ...
    def __init__(self, *, runtime_config: VisuraProcessorRuntimeConfig) -> None:
        """Зберігає runtime metadata, яка вже входить до current audit contract."""
        self.runtime_config = runtime_config

    def log_generated(self, conn, contract_id: int, out_bucket: str, out_obj: str, params_snapshot: dict[str, Any]) -> None:
    ...
    def log_failed(self, conn, contract_id: int, error: Exception | str) -> None:
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            self.audit_stage.log_generated(conn, contract_id, out_bucket, out_obj, params_snapshot)
            return output_path
        except Exception as exc:
            spider.logger.exception("[DOCX] Failed for contract %s", contract_id)
            self.audit_stage.log_failed(conn, contract_id, exc)
            return None
