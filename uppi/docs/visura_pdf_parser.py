...
class VisuraParser:
    ...
    ADDRESS_PATTERNS = {
        "via_num": re.compile(r"(?:n\.?\s*)?([\d]+[A-Z]?[\-/\dA-Z]*|SNC)", re.IGNORECASE),
        "scala": re.compile(r"(SCALA|SC\.?)\s*([A-Z0-9]+)", re.IGNORECASE),
        "interno": re.compile(r"(INTERNO|INT\.?)\s*([A-Z0-9]+)", re.IGNORECASE),
        "piano": re.compile(r"(PIANO|P\.?)\s*([-A-Z0-9°]+|T|TERRA|RIALZATO|AMMEZZATO|S\d)", re.IGNORECASE),
    }
        ...
        try:
            ...
            for page_idx in range(len(doc)):
                ...
                try:
                    ...
                    logger.exception("[VISURA_PARSER] Помилка Camelot на сторінці %d (%s): %s", page_idx + 1, pdf_path, e)
                    continue
